...
import com.duc.karaoke_app.ui.adapter.messenger.ChatRoomAdapter
import com.duc.karaoke_app.ui.adapter.messenger.OnlineUserAdapter
...
class ViewModelChat(val repository: ChatRepository, application: Application) :
    ...
    init {
        ...
        SocketManager.init(_toKenToMessenger, "http://192.168.1.12:8080")
        ...
        SocketManager.onMessageStatusUpdated { messageId, status ->
        }
    }
                ...
                if (response.isSuccessful) {
                    ...
                    messages.forEach { message ->
                        if (message.status == "sent" && message.senderId != currentUserId) {
                            Log.d("ViewModelChat", "Marking as delivered: messageId=${message.messageId}")
                            SocketManager.markAsDelivered(message.messageId, currentUserId)
                        }
                    }
                }
            } catch (e: Exception) {
